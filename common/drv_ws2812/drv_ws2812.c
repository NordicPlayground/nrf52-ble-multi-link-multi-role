...
#define LED_MATRIX_TOTAL_BIT_WIDTH  LED_MATRIX_WIDTH * LED_MATRIX_HEIGHT * 3 * 8
...
static uint32_t pwm_init(void)
{
    ...
    return nrfx_pwm_init(&m_pwm0, &pwm_config, NULL);
}
...
static void convert_rgb_to_pwm_sequence(void)
{
    ...
    uint32_t i = 0;
    for(int led = 0; led < LED_MATRIX_WIDTH * 3; led++)
    {
...
uint32_t drv_ws2812_display(void)
{
    ...
    return nrfx_pwm_simple_playback(&m_pwm0, &pwm_sequence, 1, NRFX_PWM_FLAG_STOP);
}

uint32_t drv_ws2812_pixel_draw(uint16_t x, uint16_t y, uint32_t color)
{
    if((x > LED_MATRIX_WIDTH - 1) || (y > LED_MATRIX_HEIGHT - 1))
    {
